...
export async function downloadFile(
  ...
  while (attempt <= retries) {
    try {
      ...
      if (response.status === 404) {
        ...
        return { success: false, skipped: false, bytesDownloaded: 0, error: 'Not found (404)' }
      }
      ...
      if (totalSize !== undefined && finalStats.size !== totalSize) {
        ...
        throw new Error(`Size mismatch: expected ${totalSize}, got ${finalStats.size}`)
      }
      ...
      const bytesDownloaded = isResume ? finalStats.size - existingSize : finalStats.size
  ...
  return { success: false, skipped: false, bytesDownloaded: 0, error: 'Max retries exceeded' }
}
