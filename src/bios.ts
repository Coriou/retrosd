 ...
 */
export async function downloadBios(
	biosDir: string,
		...
		try {
			...
			if (pathExists) {
				if (isSymlink) {
					...
					ui.debug(
						`${entry.system}: file exists (not a symlink)`,
						options.verbose,
					)
					results.push({
						label: `${entry.system}: file exists (kept)`,
						success: true,
					})
					continue
				}
 ...
 */
async function downloadFreeDoom(
	biosDir: string,
	options: DownloadOptions,
	...
	if (!existsSync(freedoom1Path) || !existsSync(freedoom2Path)) {
		if (existsSync(archivePath)) {
			try {
				...
				results.push({
					label: "PRBOOM: FreeDoom WADs extracted",
					success: true,
				})
			} catch (err) {
