 ...
 * ```
 */
export function useDownloader(options: DownloaderOptions | null) {
	...
	const processEvent = useCallback((event: DownloadEvent) => {
		if (event.type === "error") {
			log.download.warn(
				{
					system: event.system,
	...
	useEffect(() => {
		...
		log.download.info(
			{
				systems: Array.from(new Set(options.entries.map(e => e.system))),
				jobs: options.jobs,
